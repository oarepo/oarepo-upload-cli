...
from uct_theses.files.api import UctThesesFileRecord
from uct_theses.records.dumper import UctThesesDumper
from uct_theses.records.models import UctThesesMetadata
from uct_theses.records.multilingual_dumper import MultilingualDumper


class UctThesesIdProvider(RecordIdProviderV2):
    pid_type = "theses"


class UctThesesRecord(Record):
    model_cls = UctThesesMetadata

    schema = ConstantField("$schema", "local://uct_theses-1.0.0.json")

    index = IndexField("uct_theses-uct_theses-1.0.0")

    pid = PIDField(
        provider=UctThesesIdProvider, context_cls=PIDFieldContext, create=True
    )

    dumper_extensions = [MultilingualDumper()]
    dumper = UctThesesDumper(extensions=dumper_extensions)
    ...
    files = FilesField(store=False, file_cls=UctThesesFileRecord)
    bucket_id = ModelField()
    bucket = ModelField(dump=False)


UctThesesFileRecord.record_cls = UctThesesRecord
