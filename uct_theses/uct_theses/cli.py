...
from uct_theses.proxies import current_service
from uct_theses.records.api import UctThesesRecord


@click.group(name="uct-theses")
def group():
    pass
...
def reindex(recreate, queue, *args, **kwargs):
    if recreate:
        index = UctThesesRecord.index._name
    ...
    id_generator = (
        x[0]
        for x in db.session.query(UctThesesRecord.model_cls.id).filter(
            UctThesesRecord.model_cls.is_deleted.is_(False)
        )
    )
    ...
    if queue:
        ...
        for rec_id in tqdm.tqdm(ids, "Indexing records"):
            record = UctThesesRecord.get_record(rec_id)
